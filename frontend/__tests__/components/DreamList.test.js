...
describe("DreamList (integration with DreamCard)", () => {
  ...
  it("renders multiple items as links with correct aria-labels", () => {
    ...
    // Assert
    expect(
      screen.getByRole("link", { name: "一つ目 の詳細を見る" })
    ).toBeInTheDocument();
    expect(
      screen.getByRole("link", { name: "二つ目 の詳細を見る" })
    ).toBeInTheDocument();
  });
  ...
  it("renders only filtered dreams when a filtered list is provided", () => {
    ...
    const dreams = [
      ...
      makeDream({
        id: 2,
        title: "悲しい夢",
        emotions: [{ id: 2, name: "悲しい" }],
      }),
    ];
    const filtered = dreams.filter((d) =>
      d.emotions?.some((e) => e.name === "嬉しい")
    );
    ...
    // Assert
    expect(
      screen.getByRole("link", { name: "嬉しい夢 の詳細を見る" })
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("link", { name: "悲しい夢 の詳細を見る" })
