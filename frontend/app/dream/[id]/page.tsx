...
const Dream = () => {
  ...
  const handleUpdate = async (newTitle: string, newDescription: string) => {
    ...
    try {
      ...
      const freshData = await getDetailDream(id as string); // 新しいデータを取得
      setDetailDream(freshData); // 更新されたデータをセット
  ...
  return (
    <div className="min-h-screen py-8 px-4 md:px-12 bg-gradient-to-r from-blue-100 to-blue-300">
      <div className="bg-white p-6 rounded shadow-lg max-w-2xl mx-auto">
        <h2 className="text-4xl font-bold mb-4 text-gray-800">{isEditing ? "夢を編集" : detailDream.title}</h2>
        
        {isEditing ? (
          <form onSubmit={(e) => { e.preventDefault(); handleUpdate(detailDream.title, detailDream.description);}}>
            <div className="mb-4">
              ...
              <input
                ...
                onChange={(e) => setDetailDream({ ...detailDream, title: e.target.value })}
              />
            </div>
            <div className="mb-4">
              ...
              <textarea
                ...
                onChange={(e) => setDetailDream({ ...detailDream, description: e.target.value })}
              />
            </div>
            <div className="flex justify-between mt-4">
              <button
                type="submit"
                className={`py-2 px-4 border rounded-md ${isUpdating ? 'bg-purple-300 cursor-not-allowed' : "bg-purple-400 hover:bg-purple-500"} transition-colors duration-200 ease-in-out`}
                disabled={isUpdating}
              >
                {isUpdating ? '更新中…' : '保存'}
              </button>
          ...
          <>
            <p className="text-lg text-gray-700 md:leading-relaxed mb-8">{detailDream.description}</p>
