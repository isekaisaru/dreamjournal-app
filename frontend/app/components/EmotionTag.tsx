...
export const getChildFriendlyEmotionLabel = (originalLabel: string): string => {
  ...
  // ポジティブ (うれしい)
  if (["嬉し", "喜", "幸", "愛", "好", "希"].some(k => originalLabel.includes(k))) return "😊 うれしい";

  // 感動 (じーんとした)
  if (["感動", "感激", "胸が"].some(k => originalLabel.includes(k))) return "🥺 じーんとした";

  // ネガティブ (怒り)
  if (["怒", "腹立", "イライラ", "不満"].some(k => originalLabel.includes(k))) return "😡 おこってる";

  // ネガティブ (恐怖)
  if (["怖", "恐", "悪夢"].some(k => originalLabel.includes(k))) return "😰 こわい";

  // ネガティブ (不安・苦しい)
  if (["不安", "焦", "苦", "痛", "緊"].some(k => originalLabel.includes(k))) return "😓 しんぱい";
...
const getColorClass = (label: string): string => {
  // ポジティブ
  // ポジティブ (オレンジ)
  if (label.includes("うれしい") || ["嬉し", "喜", "幸", "愛", "好", "希"].some(k => label.includes(k))) {
  ...
  // 感動 (ピンク/ローズ系)
  if (label.includes("じーんとした") || ["感動", "感激"].some(k => label.includes(k))) {
  ...
  // 恐怖 (紫)
  if (label.includes("こわい") || ["怖", "恐", "悪夢"].some(k => label.includes(k))) {
    return "bg-purple-600 text-white border-purple-700";
  }
  // 不安 (少し薄い紫 or 紺)
  if (label.includes("しんぱい") || ["不安", "焦", "苦", "痛", "緊"].some(k => label.includes(k))) {
...
export const EmotionTag: React.FC<EmotionTagProps> = ({
  ...
  const isAlreadyMapped = ["😊", "🥺", "😡", "😰", "😓", "😢", "😌", "😲", "😵"].some(emoji => label.startsWith(emoji));
  const displayLabel = isAlreadyMapped ? label : getChildFriendlyEmotionLabel(label);
