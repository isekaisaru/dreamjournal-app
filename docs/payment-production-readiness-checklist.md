...
> **限定test-mode QAの注意**: Railsがproductionでも、test-modeのキー・Price・Webhook Eventを使う場合は `STRIPE_MODE=test` を明示する。未設定のproduction Railsはliveとして起動時検証するため、test keyとの不一致で安全に停止する。値は記録せず、modeと`livemode`の一致だけを確認する。
