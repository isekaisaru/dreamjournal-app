...
2. 各ステップは**冪等**（ensure/backfillは何度実行しても安全）。不安なら再実行してよい。
...
> 目的: 購入→Webhook→premium反映→Portal の一連が本番で1回成功すること。
> ルート: `POST /checkout`（購入セッション作成）/ `POST /billing_portal`（顧客ポータル）/ `POST /webhooks/stripe`（Webhook受信）。

手順:
- [ ] Stripeダッシュボードを**本番モード**（またはテスト用の通し確認ならテストモード）で開く
- [ ] アプリの**サブスク/課金画面**から「プレミアム」購入 → Stripe Checkout へ遷移
- [ ] （テストなら）テストカード `4242 4242 4242 4242` / 任意の未来日 / 任意CVC で決済
