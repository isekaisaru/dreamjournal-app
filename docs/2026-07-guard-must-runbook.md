...
| 項目 | 状態 | 確認根拠 |
|---|---|---|
| ① Trial P3 | 🟡 過去に成功記録あり・**正式な再QA待ち** | 修正PR #392 はマージ済み。2026年7月1日に本番URLで正しい導線（trial_login →`/home`でDB保存→ convert_trial →本登録後も夢が残る・TrialBannerが消える）まで成功した記録が作業ログにある。ただしその結果が**リポジトリやPR #392 に正式な記録として残っていない**ため、訂正済み手順で再QAして記録を残す。なお2026年7月22日のスマホ幅検証は`/trial`の「記録だけする」（DB保存されない導線）を使っており、P3の検証としては無効だった |
