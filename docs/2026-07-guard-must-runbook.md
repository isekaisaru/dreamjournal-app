...
| 項目 | 状態 | 確認根拠 |
|---|---|---|
| ① Trial P3 | 🟡 過去に成功記録あり・**正式な再QA待ち** | 修正PR #392 はマージ済み。2026年7月1日に本番URLで正しい導線（trial_login →`/home`でDB保存→ convert_trial →本登録後も夢が残る・TrialBannerが消える）まで成功した記録が作業ログにある。ただしその結果が**リポジトリやPR #392 に正式な記録として残っていない**ため、訂正済み手順で再QAして記録を残す。なお2026年7月22日のスマホ幅検証は`/trial`の「記録だけする」を使っており、**当時はDB保存されない導線だった**ためP3の検証としては無効だった（この保存不具合自体は[PR #473](https://github.com/isekaisaru/dreamjournal-app/pull/473)・commit `6febfd0`で解決済み。詳細は下記①手順内の注記参照） |
...
## ① 本番 Trial P3 実機確認（スマホ実機）— 🟡 過去に成功記録あり・正式な再QA待ち（手順を訂正済み）
...
7月OBLのKPI:
- [ ] Trial P3 本番QA完了（2026年7月1日に成功記録あり。①の訂正済み手順で再QAし、結果をここに正式記録する）
