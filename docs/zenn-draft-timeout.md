...
`url` が取れればそのまま使い、`nil` なら `b64_json` を Data URI に変換して使います。これで `gpt-image-1` のどちらのレスポンス形式にも対応できます。

**APIのレスポンス仕様はバージョンやポリシーで変わる。フォールバックを用意しておくのは本番運用の基本**だと実感した変更でした。

---

## 7. まとめ：面接でそのまま語れる設計の考え方
