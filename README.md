...
記録した夢は OpenAI API（`gpt-4o-mini`）で分析し、短い解釈文と感情タグを返します。日々の夢を単なるメモで終わらせず、親子の会話やセルフケアのきっかけに変えることを狙っています。
...
### DALL-E 3 による夢のビジュアル化

夢の内容や AI 分析結果をもとに DALL-E 3 で画像を生成し、夢の世界をビジュアルで表現できます。
Rack::Attack によるレート制限（1日3回まで）を設け、API コストの暴走を防いでいます。
...
graph TD
    ...
    DB[("PostgreSQL")]
    AI["OpenAI API<br/>(gpt-4o-mini / whisper-1 / dall-e-3)"]
    Stripe["Stripe"]
    Sentry["Sentry"]
...
パスワードポリシーはバックエンドのモデルバリデーションとフロントの入力チェックを一致させ、「8文字以上・英字と数字を各1文字以上含む」を二重に保証しています。
...
- DALL-E 3 で生成した画像の S3 永続保存（現在は OpenAI の期限付き URL）
- 画像生成・AI 分析の利用量に連動したプラン設計
...
- **AI / External Services**: OpenAI API (`gpt-4o-mini`, `whisper-1`, `dall-e-3`), Stripe, Sentry
