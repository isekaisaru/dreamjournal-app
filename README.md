...
- **モバイル / デスクトップ両対応のナビゲーション**: スマホでは親指で届く `BottomTabBar`（おうち / もり / 記録 / マイ夢 / 設定）、デスクトップでは常駐サイドバーと `⌘K` コマンドパレット（画面移動・最近の夢への即時ジャンプ）を提供し、同じ機能を各デバイスに最適な操作系で使えるようにしています。ランディングページも 2 カラムのデスクトップ向けレイアウトへ刷新しました。
...
- **セッションの永続化とトークンの digest 化**: `user_sessions` テーブルを新設し、リフレッシュトークンは digest（ハッシュ）で保存。有効期限・失効時刻・端末情報（`user_agent` / `ip_address` / `last_used_at`）を持たせ、複数端末での安全なログイン維持と個別失効を可能にしました。DB に平文トークンを残さない設計です。
