...
#
# 【背景】
# public.checkout_attempts テーブルに RLS が有効化されておらず、
...
#
# 【影響範囲】
# Rails は postgres ロール（BYPASSRLS 権限あり）で接続するため、
# RLS 有効化後も既存の checkout_attempts CRUD に影響しない。
...
class EnableRlsOnCheckoutAttempts < ActiveRecord::Migration[7.2]
  def up
    # RLS を有効化（ポリシーなし = 全行を拒否 = Default Deny）
    execute "ALTER TABLE public.checkout_attempts ENABLE ROW LEVEL SECURITY;"

    # Rails の postgres ロールは BYPASSRLS 権限を持つため引き続き全アクセス可能
    # Supabase PostgREST / anon ロールからのアクセスは遮断される
  end

  def down
    execute "ALTER TABLE public.checkout_attempts DISABLE ROW LEVEL SECURITY;"
  end
end
